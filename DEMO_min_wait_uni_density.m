...
for t = 1:stage
    t
    [~,voronoi_rg_1{t},~,~] = polybnd_voronoi(pos,bnd_pnts);
...
%     active{t}  = chooseSset(t,size(pos,1),neib1{t});
    active{t} = ones(1,n);
    l_min{t} = calcGradientMinWait_hotfix(active{t},neib1{t},voronoi_rg{t},pos,p2,size(pos,1),coef,p_int,voronoi_rg_1{t},neib2{t},adv,type,bnd_pnts);   % l_min: local minimizer
    [cst(t),~] = calcCostMinWait(neib1{t},voronoi_rg{t},pos,p2,coef,n1,adv,type,p_int,voronoi_rg_1{t},neib2{t});
    idx{t} = find(active{t});
    if type == 2 || type == 3
...
for i = 1:size(voronoi_rg{t},1)
    for j = 1:size(voronoi_rg{t},2)
        if ~isempty(voronoi_rg{t}{i,j})
            ...
            plot(voronoi_rg{t}{i,j}(:,1),voronoi_rg{t}{i,j}(:,2),'-','Color','k');
            hold on;
...
hold on;
plot3(X_int(:,1),X_int(:,2),p_int(:),'Marker','.','MarkerSize',0.5,'LineStyle','none');
plot(p_sav{t}(:,1),p_sav{t}(:,2),'Marker','o','MarkerSize',6,'MarkerFaceColor','k','Color','k','LineStyle','none');hold on;
plot(p_sav{t}(adv,1),p_sav{t}(adv,2),'Marker','o','MarkerSize',6,'MarkerFaceColor','r','Color','k','LineStyle','none'); hold on;
axis('equal')
axis([0 1 0 1]);
axis('off');
set(gca,'xtick',[]);
...
plot(1:stage,cst,'s-');xlabel('stage');ylabel('cost');



% COST PLOT
h_cst = figure('position',[0 0 500 500],'Color',[1 1 1]);
plot(1:stage,cst_aug,'s-');legend('100','100(1)','100(2)','100(3)');xlabel('stage');ylabel('cost');


%TWO
h_cst = figure('position',[0 0 500 500],'Color',[1 1 1]);
plot(1:100,cst_aug,'-');xlabel('stage');ylabel('cost');
hold on;
plot(1:100,cst_aug2,'-')

%three
h_cst = figure('position',[0 0 500 500],'Color',[1 1 1]);
plot(1:100,cst_aug,'-');legend('n=10','n=50','n=100');xlabel('stage');ylabel('cost');
hold on;
% plot(1:100,cst_aug2,'-')
