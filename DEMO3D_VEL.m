...
vmax = 0.1;       % velocity constraint is RELAXED....
stage = 50;
...
for j = 1:length(new_pos)
    ...
    plot3(new_pos{j}(size(new_pos{j},1),1),new_pos{j}(size(new_pos{j},1),2),new_pos{j}(size(new_pos{j},1),3),'Marker','o','MarkerSize',6,'MarkerFaceColor','r','Color','k','LineStyle','-');hold on;    
end
