...
function M = animation_func9vor2_fin3(p,resolution,n,f,bnd_pnts,k_p,adv)
...
%      p_appended{i}{j} = p{i} + delta_p{i} /resolution * j;
     if j >= 2
        ...
        voronoi_rg{i}{j} = polybnd_order2voronoi(p_appended{i}{j},bnd_pnts);
   end
end
...
%            [p_appended{i}{j}(m1,2) p_appended{i}{j}(m1,2)+vmax*sin(th{i}{j}(m1))]
        ...
        for i1 = 1:size(voronoi_rg{i}{j},1)
            for j1 = 1:size(voronoi_rg{i}{j},2)
                if ~isempty(voronoi_rg{i}{j}{i1,j1})
                    if ismember(i1,adv) && ismember(j1,adv)
                        patch(voronoi_rg{i}{j}{i1,j1}(:,1),voronoi_rg{i}{j}{i1,j1}(:,2),[0.5 0.5 0.5],'FaceAlpha',0.3);
                        hold on;
                    end               
                    plot(voronoi_rg{i}{j}{i1,j1}(:,1),voronoi_rg{i}{j}{i1,j1}(:,2),'-','Color','b');
                    hold on;
                end
            end
        end
